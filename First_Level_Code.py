...
input: Master .xlsx file given as first argument in command line. Additional entries are .*col name*.csv files
generated by Term_Extractor.py.

output: Assigns "A" level codes to the systems from the terms listed in the .csv file associated with a column.
Codes are assigned starting from "A1" by order of appearance in .csv file. Output is written
to *Master_File_Name*.*Column_Name*.out.xls.

Example:
input: python First_Level_Code.py SoS_Master_Copy.xlsx SoS_Master_Copy.conditions.csv
...
if __name__ == "__main__":
    try:
        ...
        for arg in sys.argv[2:]:
            ...
            #Subset surv_names to get rid of header
            surv_names = surv_names[1:]
